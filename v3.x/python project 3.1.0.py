...
def keplerstern():
    ...
    #           mx+obj.a*2,my-obj.a*2,mz-obj.a*2, mx-obj.a*2,my+obj.a*2,mz-obj.a*2, mx-obj.a*2,my-obj.a*2,mz+obj.a*2, mx-obj.a*2,my+obj.a*2,mz+obj.a*2, mx+obj.a*2,my-obj.a*2,mz+obj.a*2, mx+obj.a*2,my+obj.a*2,mz-obj.a*2, mx-obj.a*2,my-obj.a*2,mz-obj.a*2, mx+obj.a*2,my+obj.a*2,mz+obj.a*2]
    obj.xyz = [mx+obj.a,my,mz, mx,my+obj.a,mz, mx,my,mz+obj.a, mx-obj.a,my,mz, mx,my-obj.a,mz, mx,my,mz-obj.a,
...
def conv32():
    ...
    for i in range (0,len(obj.xyz)//3):
        ...
        distance = 3000
        scale = 1
        obj.xy.append((obj.xyz[3*i] * distance)/(obj.xyz[3*i+2] + distance) * scale)
        obj.xy.append((obj.xyz[3*i+1] * distance)/(obj.xyz[3*i+2] + distance) * scale)        
    return obj.xy
...
def drawfaces():
    ...
    for p in range (0,len(obj.xyz)//3):
        for q in range (0,len(obj.xyz)//3):
            ...
            if (abs(pq-obj.sl) < 10) or (abs(pq-obj.sl/3*math.sqrt(2)) < 10):
                for r in range (0,len(obj.xyz)//3):
                    ...
                    if (abs(pr-obj.sl) < 10) or (abs(pr-obj.sl/3*math.sqrt(2)) < 10):
                        qr = math.sqrt((obj.xyz[3*q]-obj.xyz[3*r])**2+(obj.xyz[3*q+1]-obj.xyz[3*r+1])**2+(obj.xyz[3*q+2]-obj.xyz[3*r+2])**2)
                        if (abs(qr-obj.sl) < 10) or (abs(qr-obj.sl/3*math.sqrt(2)) < 10):
                            if obj.ek > 4:
                                print('ek > 4')
                            else:
                                #p,q,r sortieren
...
cbreprot.grid(row = 5, column = 1, columnspan = 2, padx = 15, pady = 15)

buttonpause = Button(master, text = 'abbrechen', command = lambda:(cbreprot.deselect()))
buttonpause.grid(row = 5, column = 3, padx = 15, pady = 15)
