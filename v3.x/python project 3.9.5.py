...
def mzsort(list):
    for m in range(0,len(list)//5):
            for n in range(0,len(list)//5):
                mzpolygonm = (obj.xyz[3*list[5*m]+2] + obj.xyz[3*list[5*m+1]+2] + obj.xyz[3*list[5*m+2]+2] + obj.xyz[3*list[5*m+3]+2] + obj.xyz[3*list[5*m+4]+2])/5
                mzpolygonn = (obj.xyz[3*list[5*n]+2] + obj.xyz[3*list[5*n+1]+2] + obj.xyz[3*list[5*n+2]+2] + obj.xyz[3*list[5*n+3]+2] + obj.xyz[3*list[5*n+4]+2])/5
                if (mzpolygonm > mzpolygonn) and (m < n):
                    list[5*m],list[5*n] = list[5*n],list[5*m]
                    list[5*m+1],list[5*n+1] = list[5*n+1],list[5*m+1]
                    list[5*m+2],list[5*n+2] = list[5*n+2],list[5*m+2]
                    list[5*m+3],list[5*n+3] = list[5*n+3],list[5*m+3]
                    list[5*m+4],list[5*n+4] = list[5*n+4],list[5*m+4]
    return(facelist)

def dupedetect(p,q,r,s,t):
    #p,q,r sortieren
    abc = [p,q,r,s,t]
    for f in range(0,5):
        for g in range(0,5):
    ...
    if len(facelist) == 0:
        ...
        add = 1
        for i in range(0,len(facelist)//5):
            if (facelist[5*i] == abc[0]) and (facelist[5*i+1] == abc[1]) and (facelist[5*i+2] == abc[2]) and (facelist[5*i+3] == abc[3]) and (facelist[5*i+4] == abc[4]):
                add = 0
        if add == 1:
            #ggf. hinzufügen
            for j in range(0,5):
...
def drawfaces():
                                ...
                                if (abs(qs-obj.sl) < 10):
                                    for t in range (0,len(obj.xyz)//3):
                                        ...
                                        if (abs(st-obj.sl) < 10):
                                            ...
                                            if (abs(rt-obj.sl) < 10):
                                                if p != q and p != r and p != s and p != t and q != r and q != s and q != t and r != s and r != t and s != t:
                                                    abc = [p,q,r,s,t]
                                                    abctemp = abc
                                                    for f in range(0,5):
                                                        for g in range(0,5):
                                                            if (abctemp[f] > abctemp[g]) and (f < g):
                                                                abctemp[f],abctemp[g] = abctemp[g],abctemp[f]
                                                    if len(facesortedlist) == 0:
                                                        for j in range (0,5):
                                                            facesortedlist.append(abctemp[j])
                                                            facelist.append(abc[j])
                                                    else:
                                                        add = 1
                                                        for i in range(0,len(facelist)//5):
                                                            if (facesortedlist[5*i] == abctemp[0]) and (facesortedlist[5*i+1] == abctemp[1]) and (facesortedlist[5*i+2] == abctemp[2]) and (facesortedlist[5*i+3] == abctemp[3]) and (facesortedlist[5*i+4] == abctemp[4]):
                                                                add = 0
                                                        if add == 1:
                                                        #ggf. hinzufügen
                                                            for j in range(0,5):
    ...
    for i in range(0,len(facelist)//5):
        ...
        if obj.ek == 5:
            canvas1.create_polygon(obj.xy[2*p],obj.xy[2*p+1], obj.xy[2*s],obj.xy[2*s+1], obj.xy[2*t],obj.xy[2*t+1],obj.xy[2*q],obj.xy[2*q+1], obj.xy[2*r],obj.xy[2*r+1], fill = getcolor('fc'), outline = getcolor('oc'))
