...
def dupedetect(p,q,r,s,t):
    ...
    if len(facelist) == 0:
        listi.append(0)
        for i in range(0,5):
            facelist.append(abctemp[i])

    else:
        #prüfen ob p,q,r in liste enthalten ist
        add = 1
        for i in range(0,len(facelist)//5):
            if (facelist[5*i] == abctemp[0]) and (facelist[5*i+1] == abctemp[1]) and (facelist[5*i+2] == abctemp[2]) and (facelist[5*i+3] == abctemp[3]) and (facelist[5*i+4] == abctemp[4]):
                add = 0
        if add == 1:
            for j in range (0,5):
                facelist.append(abctemp[j])
            listi.append(i)
        if obj.ek == 3:
            listi = listi[0:20]
