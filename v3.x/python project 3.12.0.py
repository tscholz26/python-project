...
versionshinweis =  '(Version 3.12.0)'
...
def gradientgetcolor():
    ...
    # 0 und 2 sind vertauscht, um farbverlauf bei blau beginnen zu lassen
    colorrgb = 'rgb(' + str(colors[2]) + ',' + str(colors[1]) + ',' + str(colors[0]) + ')'
...
def inffuncgradient():
    time.sleep(0.001)
    master.after(50, lambda:(drawgradient(), inffuncgradient()))
...
def drawfaces():
                                        ...
                                        for t in range (0,len(obj.xyz)//3):
                                            ...
                                            if (abs(st-obj.sl) < 10):
                                                ...
                                                if (abs(rt-obj.sl) < 10):
                                                    obj.xy = conv32()
                                                    provebvb()
                                                    canvas1.create_polygon(obj.xy[2*p],obj.xy[2*p+1], obj.xy[2*q],obj.xy[2*q+1], obj.xy[2*s],obj.xy[2*s+1],obj.xy[2*t],obj.xy[2*t+1], obj.xy[2*r],obj.xy[2*r+1], fill = fccolor, outline = occolor)
    ...
    if obj.ek == 3:
        ...
        for i in range(0,len(facelist)//3):
            ...
            r = facelist[3*i+2]
            provebvb()
            canvas1.create_polygon(obj.xy[2*p],obj.xy[2*p+1], obj.xy[2*q],obj.xy[2*q+1], obj.xy[2*r],obj.xy[2*r+1], fill = fccolor, outline = occolor) 
...
cbgradient = Checkbutton(master, text = 'Farbverlauf', var = checkgradient, command = initgradient)
cbgradient.grid(row = 2, column = 16, columnspan = 1)

scalegradient = Scale(master, highlightthickness = 0, from_ = 50, to = 800, orient = 'horizontal', showvalue = 0)
scalegradient.grid(row = 2, column = 17, padx = 15)
