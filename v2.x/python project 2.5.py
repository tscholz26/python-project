                        ...
                        if (abs(qr-obj.sl) < 10):
                            if obj.ek > 3:
                                for s in range (0,len(obj.xyz)//3):
                                    ps = math.sqrt((obj.xyz[3*p]-obj.xyz[3*s])**2+(obj.xyz[3*p+1]-obj.xyz[3*s+1])**2+(obj.xyz[3*p+2]-obj.xyz[3*s+2])**2)
                                    if (abs(ps-obj.sl) < 10):
                                        rs = math.sqrt((obj.xyz[3*r]-obj.xyz[3*s])**2+(obj.xyz[3*r+1]-obj.xyz[3*s+1])**2+(obj.xyz[3*r+2]-obj.xyz[3*s+2])**2)
                                        if (abs(rs-obj.sl) < 10):
                                            ...
                                            if (abs(qs-obj.sl) < 10):
                                                if obj.ek > 4:
                                                    for t in range (0,len(obj.xyz)//3):
                                                        pt = math.sqrt((obj.xyz[3*p]-obj.xyz[3*t])**2+(obj.xyz[3*p+1]-obj.xyz[3*t+1])**2+(obj.xyz[3*p+2]-obj.xyz[3*t+2])**2)
                                                        if (abs(pt-obj.sl) < 10):
                                                            qt = math.sqrt((obj.xyz[3*q]-obj.xyz[3*t])**2+(obj.xyz[3*q+1]-obj.xyz[3*t+1])**2+(obj.xyz[3*q+2]-obj.xyz[3*t+2])**2)
                                                            if (abs(qt-obj.sl) < 10):
                                                                ...
                                                                if (abs(rt-obj.sl) < 10):
                                                                    ...
                                                                    if (abs(st-obj.sl) < 10):
                                                                        #umwandeln in 2d
                                                                        obj.xy = conv32()
                                                                        canvas1.create_polygon(obj.xy[2*p],obj.xy[2*p+1],obj.xy[2*q],obj.xy[2*q+1],obj.xy[2*r],obj.xy[2*r+1],obj.xy[2*s],obj.xy[2*s+1],obj.xy[2*t],obj.xy[2*t+1], fill = 'yellow', outline = 'blue')

                                                                        
                                                else:
                                                    #umwandeln in 2d
                                                    obj.xy = conv32()
                                                    canvas1.create_polygon(obj.xy[2*p],obj.xy[2*p+1],obj.xy[2*q],obj.xy[2*q+1],obj.xy[2*r],obj.xy[2*r+1],obj.xy[2*s],obj.xy[2*s+1], fill = 'yellow', outline = 'blue')
