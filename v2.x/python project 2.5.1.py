...
def conv32():
    ...
    print('Umwandlung in 2d abgeschlossen')
    return obj.xy
    ...
    for p in range (0,len(obj.xyz)//3):
        ...
        for q in range (0,len(obj.xyz)//3):
            if (p != q):
                ...
                yq = obj.xyz[2*q+1]
                zq = obj.xyz[2*q+2]
                pq = math.sqrt((xp-xq)**2+(yp-yq)**2+(zp-zq)**2)
                if (abs(pq-obj.sl) < 5):
                    for r in range (0,len(obj.xyz)//3):
                        if (r != p) and (r != q):
                            ...
                            yr = obj.xyz[2*r+1]
                            zr = obj.xyz[2*r+2]
                            pr = math.sqrt((xp-xr)**2+(yp-yr)**2+(zp-zr)**2)
                            if (abs(pr-obj.sl) < 5):
                                xq = obj.xyz[2*q]
                                yq = obj.xyz[2*q+1]
                                zq = obj.xyz[2*q+2]
                                qr = math.sqrt((xq-xr)**2+(yq-yr)**2+(zq-zr)**2)
                                if (abs(qr-obj.sl) < 5):
                                    if obj.ek > 4:
                                        print('ek > 4')
                                    else:
                                        ...
                                        if (pqr in facelist):
                                            print('Duplikat erkannt')
                                        else:
                                            facelist.append(pqr)
                                            print('Neues Dreieck erkannt')
                                            obj.xy = conv32()
                                            #finales zeichnen
